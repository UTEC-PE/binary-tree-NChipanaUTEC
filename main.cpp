...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < numberOfElements; i++) {
        insertIntoTree(tester); // Tu insert no está bien implementado
    }
    sortAndPruneHelper();
...
void insertIntoTree(Tree &tester) {
    ...
    tester.insert(numberToInsert, tester.get_root());
}
...
void removeFromTree(Tree &tester) {
    ...
    tester.remove(element, tester.get_root());
}
